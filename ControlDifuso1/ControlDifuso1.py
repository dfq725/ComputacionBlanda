...
#servicio y de la comida en restaurante
...
propina_alta = fuzz.trimf(x_propina, [13, 13, 25])
...
# El operador OR significa que tomamos el maximo de estas dos.
activar_regla1 = np.fmax(nivel_calidad_bajo, nivel_servicio_bajo)

# Ahora aplocamos esto recortando la parte superior de la salida correspondiente
# Funcion de membresia con 'np.fmin'
activacion_propina_baja = np.fmin(activar_regla1, propina_baja)    # Eliminando por completo a 0
...
# Cancela los ejes seuperior / derecho
for ax in (ax0,):
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.get_xaxis().tick_bottom()
